...
void VisualizeConstrainPlanes(g2o::ellipsoid& e_local, g2o::SE3Quat& Twc, Map* pMap)
{
    ...
    for( int i=0; i< planeNum; i++)
    {
        ...
        // else 
        {
            ...
            if(vCPlanes[i]->image_border) color = Vector3d(0.7,0,0);  // 边界颜色
            else color = Vector3d(0.3,1,0); // 非边界
        }
...
{
    Vector3d axis(0,0,1);
    int id = -1; double max_angle = -CV_PI;
...
{
    Vector3d axis(0,0,1);
    int id = -1; double min_angle = CV_PI;
    int num = vecPlanes.size();
    for(int i = 0; i<num; i++)
    {
        g2o::plane* pl = vecPlanes[i];
...
std::vector<bool> GetBorderFlags(Vector4d &measure, int rows, int cols)
{
    ...
    // -----
    std::vector<bool> flags; flags.resize(4);
...
{   
    ...
    for(auto ppl : vCPlanesWithBorderFlags){
        ...
        if(mCamera_fx<0) center_bbox_3d[0] = -center_bbox_3d[0];
        if(mCamera_fy<0) center_bbox_3d[1] = -center_bbox_3d[1];
...
{
    MatrixXd outputMat; outputMat.resize(0, 4);
    for(auto ppl : vecPlanes)
    {
        VectorXd param(4); param << ppl->param;
                                ...
                                camera_intrinsic &camera, pcl::PointCloud<PointType>::Ptr& pcd_ptr)
{
    ...
    // ✅ 5. 将点云转换到重力坐标系
    // 获得该系下的点云.
    // std::cout<< " [debug] EstimateLocalEllipsoidUsingMultiPlanes 2-2, 当前帧中的物体点云中心:"<< centroid.transpose() << std::endl;
...
g2o::ellipsoid EllipsoidExtractor::EstimateEllipsoidFromPCDCloud(pcl::PointCloud<PointType>::Ptr& pcd_ptr, g2o::plane* ground, int ManualDirection)
{
    ...
    pcl::PointCloud<PointType>::Ptr pCloudPCL = pcd_ptr;
    ...
    VectorXd sup_plane = ground->param;    
    // 计算物体点云中心；
    Eigen::Vector4d centroid; pcl::compute3DCentroid(*pCloudPCL, centroid);
    // 构造从世界坐标系到重力坐标系的变换 Twg：
    g2o::SE3Quat Twg = GenerateGravityCoordinate(centroid.head(3), sup_plane.head(3));
    ...
    // 可视化: 重力系下的物体
    ORB_SLAM2::PointCloud* pObjectCloudGravity = pclXYZToQuadricPointCloudPtr(pCloudPCLGravity); // normalized coordinate
    mpMap->AddPointCloudList("ObjectPCDCloud - Newest Merged", pObjectCloudGravity, 0);
    // delete pObjectCloudGravity; pObjectCloudGravity = NULL;
